...
class SliceAlgorithmTest(unittest.TestCase):
    ...
    def _create_axis_dict(
        self, units="DeltaE", start=-10, end=15, step=1, e_unit="meV"
    ):
        return {
            "units": self.MockProperty(units),
            "start": self.MockProperty(start),
            "end": self.MockProperty(end),
            "step": self.MockProperty(step),
            "e_unit": self.MockProperty(e_unit),
        }
    ...
    def test_PyExec_nonPSD(
        self,
        mock_get_property,
        mock_energy_units,
        mock_compute_nonPSD,
        mock_attribute_to_log,
        mock_set_property,
    ):
        x_dict = self._create_axis_dict()
        y_dict = self._create_axis_dict(units="|Q|", start=0.1, end=3.1, step=0.1)
        self.test_objects = self._create_tst_objects(
            self.sim_scattering_data, x_dict, y_dict
        )
        ...
        mock_set_property.assert_called_with(
            "OutputWorkspace", mock_compute_nonPSD.return_value
        )
    ...
    @patch("mslice.models.slice.slice_algorithm.Slice._compute_slice_nonPSD")
    @patch("mslice.models.slice.slice_algorithm.PythonAlgorithm.getProperty")
    def test_PyExec_nonPSD_non_meV(
        self,
        mock_get_property,
        mock_compute_nonPSD,
        mock_ScaleX,
        ...
        y_dict = self._create_axis_dict(
            units="|Q|", start=0.1, end=3.1, step=0.1, e_unit="cm-1"
        )
        self.test_objects = self._create_tst_objects(
            self.sim_scattering_data, x_dict, y_dict
        )
        ...
        test_slice.PyExec()
        mock_compute_nonPSD.assert_called_once()
        mock_ScaleX.assert_called_with(
            InputWorkspace=mock_compute_nonPSD.return_value,
            Factor=8.065544,
            Operation="Multiply",
            StoreInADS=False,
        )
        mock_attribute_to_log.assert_called_once()
        mock_set_property.assert_called_with(
            "OutputWorkspace", mock_ScaleX.return_value
        )
    ...
    def test_PyExec_PSD(
        ...
        y_dict = self._create_axis_dict(units="|Q|", start=0.1, end=3.1, step=0.1)
        self.test_objects = self._create_tst_objects(
            self.sim_scattering_data, x_dict, y_dict, PSD=True
        )
        mock_get_property.side_effect = self._property_side_effect
        mock_energy_units.return_value.factor_from_meV.return_value = 1.0
        ...
        mock_attribute_to_log.assert_called_once()
        mock_set_property.assert_called_with(
            "OutputWorkspace", mock_compute_PSD.return_value
        )
    ...
    def test_PyExec_PSD_non_meV(
        ...
        test_slice.PyExec()
        mock_compute_PSD.assert_called_once()
        mock_transform_MD.assert_called_with(
            InputWorkspace=mock_compute_PSD.return_value, Scaling=[8.065544, 1.0]
        )
        mock_attribute_to_log.assert_called_once()
        mock_set_property.assert_called_with(
            "OutputWorkspace", mock_transform_MD.return_value
        )
    ...
    @patch("mslice.models.slice.slice_algorithm.TransformMD")
    @patch("mslice.models.slice.slice_algorithm.PythonAlgorithm.getProperty")
    def test_PyExec_PSD_non_meV_with_DeltaE_y_axis(
        self,
        mock_get_property,
        mock_transform_MD,
        mock_compute_PSD,
        ...
        test_slice.PyExec()
        mock_transform_MD.assert_called_with(
            InputWorkspace=mock_compute_PSD.return_value, Scaling=[1.0, 8.065544]
        )

    def test_dimension_index_initial_success(self):
        workspace = MagicMock()
        workspace.getDimensionIndexByName.side_effect = (
            self._getDimensionIndexByName_side_effect
        )
        axis = MagicMock()
        axis.units = "test"
    ...
    def test_dimension_index_error(self):
        ...
        test_slice = Slice()
        self.assertRaises(
            RuntimeError, lambda: test_slice.dimension_index(workspace, axis)
        )
    ...
    def test_compute_slice_PSD(self, mock_get_no_steps):
        ...
        test_slice = Slice()
        computed_slice = test_slice._compute_slice_PSD(
            workspace, mock_x_axis, mock_y_axis, None
        )

        self.assertTrue(isinstance(computed_slice, MDHistoWorkspace))
        self.assertEqual(
            computed_slice.getSignalArray().shape,
            (mock_get_no_steps.return_value, mock_get_no_steps.return_value),
        )
        self.assertEqual(
            np.sum(computed_slice.getNumEventsArray()), workspace.getNPoints()
        )
    ...
    def test_compute_slice_nonPSD_direct(self):
        ...
        test_slice = Slice()
        computed_slice = test_slice._compute_slice_nonPSD(
            self.test_objects["workspace"].raw_ws,
            x_axis,
            y_axis,
        ...
        self.assertEqual(
            computed_slice.blocksize(),
            (x_dict["end"].value - x_dict["start"].value) / x_dict["step"].value,
        )
        self.assertEqual(
            computed_slice.getNumberHistograms(),
            (y_dict["end"].value - y_dict["start"].value) / y_dict["step"].value,
        )
        self.assertEqual(
            computed_slice.getNPoints(),
            self.test_objects["workspace"].raw_ws.getNPoints(),
        )
    ...
    def test_compute_slice_nonPSD_indirect(self, mock_SofQW3):
        ...
        y_dict = self._create_axis_dict(units="|Q|", start=0.1, end=3.1, step=0.1)
        self.test_objects = self._create_tst_objects(
            self.sim_scattering_data, x_dict, y_dict, e_mode="Indirect"
        )
        x_axis, y_axis = self._generate_axis(x_dict, y_dict)
        ...
        test_slice = Slice()
        test_slice._compute_slice_nonPSD(
            raw_ws,
            x_axis,
            y_axis,
            self.test_objects["workspace"].e_mode,
        ...
        mock_SofQW3.assert_called_once_with(
            InputWorkspace=ANY,
            QAxisBinning=ANY,
            EAxisBinning=ANY,
            EMode=ANY,
    ...
    def test_compute_slice_nonPSD_error_if_no_DeltaE_axis(self):
        ...
        self.assertRaises(
            RuntimeError,
            lambda: test_slice._compute_slice_nonPSD(
                self.test_objects["workspace"].raw_ws,
                x_axis,
                y_axis,
                self.test_objects["workspace"].e_mode,
