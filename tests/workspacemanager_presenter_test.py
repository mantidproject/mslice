...
class WorkspaceManagerPresenterTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.sim_workspace = CreateSimulationWorkspace(
            Instrument="MAR",
            BinParams=[-10, 1, 10],
            UnitX="DeltaE",
            OutputWorkspace="ws0",
        ...
        cls.px_workspace = ConvertToMD(
            InputWorkspace=cls.sim_workspace,
            OutputWorkspace="ws1",
            QDimensions="|Q|",
        ...
        cls.px_workspace_clone = CloneWorkspace(
            InputWorkspace=cls.px_workspace, OutputWorkspace="ws2", StoreInADS=False
        )
    ...
    def test_save_workspace_cancelled(self, save_ws_mock, save_dir_mock):
        ...
        # to save to on calls to get_workspace_to_save_filepath
        path_to_save_to = (
            ""  # view returns empty string to indicate operation cancelled
        )
        workspace_to_save = "file1"
        self.view.get_workspace_selected = mock.Mock(return_value=[workspace_to_save])
    ...
    @patch(
        "mslice.plotting.globalfiguremanager.GlobalFigureManager.get_plotted_windows_dict"
    )
    ...
    @patch("mslice.presenters.workspace_manager_presenter.delete_workspace")
    def test_remove_multiple_workspaces(
        self, delete_ws_mock, mock_get_plotted_windows_dict
    ):
        self.presenter = WorkspaceManagerPresenter(self.view)
        ...
        workspace1 = CloneWorkspace(
            self.m_workspace.raw_ws, OutputWorkspace=sel_ws_1_name
        )
        workspace2 = CloneWorkspace(
            self.m_workspace.raw_ws, OutputWorkspace=sel_ws_2_name
        )
    ...
    @patch("mslice.presenters.workspace_manager_presenter.delete_workspace")
    def test_remove_multiple_workspaces_not_plotted(
        self, delete_ws_mock, mock_get_plotted_windows_dict
    ):
        self.presenter = WorkspaceManagerPresenter(self.view)
        ...
        workspace_instances = [
            CloneWorkspace(
                self.m_workspace.raw_ws, OutputWorkspace=selected_ws_names[i]
            )
            for i in range(3)
        ]
        for i in range(3):
            add_workspace(workspace_instances[i], selected_ws_names[i])
        self.view.get_workspace_selected = mock.Mock(return_value=[*selected_ws_names])
    ...
    def test_save_to_ads(self, export_to_ads_mock):
        ...
        export_to_ads_mock.assert_has_calls(
            [mock.call(ws) for ws in selected_workspaces]
        )
    ...
    def test_rebose_workspace(self, scale_ws_mock):
        ...
        self.view.scale_input.assert_called()
        self.assertTrue(not self.view.error_select_more_than_one_workspaces.called)
        scale_ws_mock.assert_called_once_with(
            selected_workspaces, from_temp=250, to_temp=2
        )
