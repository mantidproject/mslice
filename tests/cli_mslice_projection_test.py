...
class CLIProjectionTest(unittest.TestCase):
    ...
    def test_that_mslice_projection_errorbar_works_correctly(
        self, is_gui, plot_cut, errorbar
    ):
    ...
    def test_that_mslice_projection_pcolormesh_works_correctly(
        self, plot_slice, pcolormesh
    ):
    ...
    def test_that_recoil_works_as_expected_without_relative_molecular_mass(
        self, get_spp, update_legend, update_check
    ):
        element = "Hydrogen"
        ...
        get_spp().add_overplot_line.assert_called_with(
            self.workspace.name, key, recoil=True, cif=None
        )
    ...
    @mock.patch("mslice.cli._update_legend")
    @mock.patch("mslice.app.presenters.get_slice_plotter_presenter")
    def test_that_recoil_works_as_expected_with_relative_molecular_mass(
        self, spp, update_legend, update_check, gfm
    ):
        rmm = 34
        ...
        spp().add_overplot_line.assert_called_with(
            self.workspace.name, key, recoil=True, cif=None
        )
        update_legend.assert_called_once_with()
        update_check.assert_called_with(key)
    ...
    @mock.patch("mslice.cli._update_legend")
    @mock.patch("mslice.app.presenters.get_slice_plotter_presenter")
    def test_that_bragg_works_as_expected_without_cif(
        self, get_spp, update_legend, update_check
    ):
        element = "Tantalum"
        key = _get_overplot_key(element, rmm=None)
        ...
        get_spp().add_overplot_line.assert_called_with(
            self.workspace.name, key, recoil=False, cif=None
        )
        update_legend.assert_called_once_with()
        update_check.assert_called_with(key)
    ...
    def test_that_waterfall_command_works(self):
        ...
        ax = fig.add_subplot(111, projection="mslice")
        with mock.patch(
            "mslice.cli._mslice_commands.GlobalFigureManager.get_active_figure"
        ) as gaf:
