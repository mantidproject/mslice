...
class CutPlotterPresenterTest(unittest.TestCase):
    ...
    def test_plot_interactive_cut_removes_hidden_ads_workspaces(
        ...
        current_interactive_ws_name_pair = (
            "__previous_cut(1.264,3.001)_HIDDEN",
            "__previous_cut(1.264,3.001)",
        )
        self.cut_plotter_presenter.interactive_ws_names = (
            current_interactive_ws_name_pair
        )
        self.cut_plotter_presenter.plot_interactive_cut("workspace", cut, False, False)

        remove_from_ads_mock.assert_called_once_with(
            current_interactive_ws_name_pair[0]
        )
        self.assertEqual(
            self.cut_plotter_presenter.interactive_ws_names,
            (cut.workspace_raw_name, cut.workspace_name),
        )
