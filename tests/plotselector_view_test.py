...
class PlotSelectorWidgetTest(unittest.TestCase):
    def setUp(self):
        ...
        self.presenter.get_plot_name_from_number = mock.Mock(
            side_effect=self.se_plot_name
        )
        self.presenter.get_initial_last_active_value = mock.Mock(
            side_effect=self.se_get_initial_last_active_value
        )
        self.presenter.is_shown_by_filter = mock.Mock(
            side_effect=self.se_is_shown_by_filter
        )
    ...
    def test_getting_all_selected_plot_numbers_with_nothing_selected_returns_empty_list(
        self,
    ):
        selected_plots = self.view.get_all_selected_plot_numbers()
    ...
    def test_getting_currently_selected_plot_number_with_nothing_selected_returns_None(
        self,
    ):
        plot_numbers = [0, 1, 2]
        self.view.set_plot_list(plot_numbers)
    ...
    def test_plot_name_double_clicked_calls_presenter_and_makes_plot_current(self):
        ...
        QTest.mouseClick(
            self.view.table_widget.viewport(), Qt.LeftButton, pos=item_center
        )
        QTest.mouseDClick(
            self.view.table_widget.viewport(), Qt.LeftButton, pos=item_center
        )
    ...
    def test_set_sort_by_number(self):
        ...
        self.assert_list_of_plots_is_set_in_widget(
            ["Plot1", "Plot2", "Plot3", "Plot20", "Graph99"]
        )
    ...
    def test_sorting_by_name(self):
        ...
        self.view.set_plot_list([0, 1, 2, 42, 19])
        self.assert_list_of_plots_is_set_in_widget(
            ["Graph99", "Plot1", "Plot2", "Plot3", "Plot20"]
        )
    ...
    def test_set_sort_by_name_descending(self):
        ...
        self.view.set_sort_order(False)
        self.assert_list_of_plots_is_set_in_widget(
            ["Plot20", "Plot3", "Plot2", "Plot1", "Graph99"]
        )
    ...
    def test_sort_by_last_active(self):
        ...
        self.view.set_last_active_values(
            {0: 2, 1: 1, 2: "_Plot3", 42: "_Graph99", 19: "_Plot20"}
        )

        self.assert_list_of_plots_is_set_in_widget(
            ["Plot2", "Plot1", "Graph99", "Plot3", "Plot20"]
        )
    ...
    def adding_to_list_with_sorting_by_name(self):
        ...
        self.assert_list_of_plots_is_set_in_widget(
            ["Graph99", "Plot1", "Plot2", "Plot3", "Plot15", "Plot20"]
        )
    ...
    def adding_to_list_with_sorting_by_last_active(self):
        ...
        self.assert_list_of_plots_is_set_in_widget(
            ["Plot2", "Plot1", "Graph99", "Plot3", "Plot15", "Plot20"]
        )
    ...
    def test_export_button_pressed(self):
        ...
        for i in range(len(EXPORT_TYPES)):
            self.assertEqual(
                self.presenter.export_plots_called.mock_calls[i],
