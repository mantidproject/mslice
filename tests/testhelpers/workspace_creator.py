...
def create_simulation_workspace(e_mode: str, output_name: str, psd: bool = False):
    ...
    if e_mode == "Direct":
        workspace = mantid_algorithms.CreateSimulationWorkspace(
            Instrument="MAR",
            BinParams=[-0.15, 0.01, 0.15],
            UnitX="DeltaE",
...
def create_workspace(output_name: str):
    ...
    workspace = mantid_algorithms.CreateSampleWorkspace(
        NumBanks=1,
        BankPixelWidth=5,
        XMin=0.1,
        XMax=3.1,
...
def create_md_workspace(n_dims: int, output_name: str):
    ...
    FakeMDEventData(
        ...
        PeakParams=f"500000,{','.join(['0'] * n_dims)},3",
...
def create_pixel_workspace(n_dims: int, output_name: str) -> PixelWorkspace:
    ...
    md_workspace = CreateMDWorkspace(
        Dimensions=n_dims,
        Extents=_create_extents(n_dims),
        Units=_create_units(n_dims),
        Names=_create_dim_names(n_dims),
