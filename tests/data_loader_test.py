...
class DataLoaderTest(unittest.TestCase):
    ...
    def test_load_one_workspace(self, get_ws_handle_mock, load_mock, process_limits):
        ...
        with patch(
            "mslice.models.workspacemanager.workspace_algorithms.get_workspace_handle"
        ) as gwh:
            gwh.return_value = ws_mock
            limits = PropertyMock(
                side_effect=({} if i < 2 else {"DeltaE": [-1, 1]} for i in range(6))
            )
        ...
        load_mock.assert_called_with(
            filename=path_to_nexus, output_workspace=workspace_name
        )
    ...
    @patch("mslice.presenters.data_loader_presenter.get_workspace_handle")
    def test_load_multiple_workspaces(
        self, get_ws_handle_mock, get_ws_names_mock, load_mock
    ):
        ...
        self.view.confirm_overwrite_workspace = mock.Mock(
    ...
    def test_load_and_merge(self, get_ws_handle_mock, load_mock):
        ...
        load_mock.assert_called_once_with(
            filename=path1 + "+" + path2 + "+" + path3, output_workspace="file1_merged"
        )
