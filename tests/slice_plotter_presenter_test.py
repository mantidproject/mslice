...
class SlicePlotterPresenterTest(unittest.TestCase):
    ...
    def test_plot_slice_success(
        self,
        compute_slice_mock,
        get_workspace_handle_mock,
        create_slice_mock,
        ...
        slice_presenter = SlicePlotterPresenter()
        slice_presenter.plot_slice(
            "workspace", x_axis, y_axis, 7, 8, False, DEFAULT_CMAP
        )
    ...
    def test_add_overplot_line(
        self, compute_powder_mock, compute_recoil_mock, plot_line_mock
    ):
        slice_presenter = SlicePlotterPresenter()
        ...
        plot_line_mock.assert_called_with(
            [1], np.array([2]) * 8.065544, recoil_key, True, cache_mock
        )
        ...
        plot_line_mock.assert_called_with(
            [3], np.array([4]) * 8.065544, powder_key, False, cache_mock
        )
