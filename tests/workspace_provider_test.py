...
class MantidWorkspaceProviderTest(unittest.TestCase):
    def setUp(self):
        ...
        AddSampleLog(
            Workspace=self.test_ws_2d.raw_ws,
            LogName="Ei",
            LogText="50.",
            LogType="Number",
            StoreInADS=False,
        )
        self.test_ws_md = ConvertToMD(
            OutputWorkspace="test_ws_md",
    ...
    def test_combine_workspace(self, step_mock):
        ...
        np.testing.assert_array_almost_equal(
            combined.limits["|Q|"], [0.2939, 9.4817, 0.0919], 4
        )
    ...
    def test_that_rename_workspace_works_as_expected(
        self, get_ws_names_mock, rename_ws_mock
    ):
    ...
    def test_propagate_properties(self):
        ws_2 = CreateSimulationWorkspace(
            OutputWorkspace="test_ws_2",
            Instrument="MAR",
            BinParams=[-1, 1, 20],
            UnitX="DeltaE",
        )
    ...
    def test_get_limits_100_steps(self, step_mock):
        ...
        np.testing.assert_allclose(
            limits, [0.545576, 8.615743, 0.042867], rtol=0, atol=1e-3
        )
    ...
    def test_get_limits_saved(self):
        ...
        np.testing.assert_array_equal(
            self.test_ws_2d.limits["|Q|"], self.test_ws_2d.limits["MomentumTransfer"]
        )
        np.testing.assert_almost_equal(
            self.test_ws_2d.limits["|Q|"], [0.25116, 9.52454, 0.04287], 5
        )
        np.testing.assert_almost_equal(
            self.test_ws_2d.limits["2Theta"], [3.43, 134.14, 0.57296], 5
        )
