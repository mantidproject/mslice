...
class CommandLineTest(unittest.TestCase):
    ...
    def test_load(self, ospath_mock, get_dlp, get_workspace_mock):
        ...
        get_dlp().load_workspace.assert_called_once_with(
            [path], False, force_overwrite=-1
        )
    ...
    def test_load_merge(self, ospath_mock, get_dlp, get_workspace_mock):
        ...
        get_dlp().load_workspace.assert_called_once_with(
            [path], True, force_overwrite=-1
        )
        get_workspace_mock.assert_called_with(path)

    @mock.patch("mslice.app.presenters.get_powder_presenter")
    def test_projection(self, get_pp):
        get_pp.return_value = PowderProjectionPresenter(
            mock.create_autospec(PowderView), MantidProjectionCalculator()
        )
    ...
    def test_that_generate_script_works_as_expected(self, gfm, generate_script):
        ...
        generate_script.assert_called_once_with(
            ws_name=workspace.name[2:], filename=filename, plot_handler=plot_handler
        )
    ...
    def test_plot_cut(self, cpp, is_gui):
        ...
        cut = Cut(workspace)
        PlotCut(cut)
        cpp.plot_cut_from_workspace.assert_called_once_with(
            cut, intensity_range=(None, None), plot_over=False
        )
