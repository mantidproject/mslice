...
class CutAlgorithmTest(TestCase):
    ...
    def xtest_that_compute_cut_returns_a_result_with_the_expected_size_for_normalized_psd_rebin_data(
        self,
    ):
        normalized = True
        algorithm = "Rebin"
    ...
    def test_that_compute_cut_returns_a_result_with_the_expected_size_for_normalized_non_psd_rebin_data(
        self,
    ):
        normalized = True
        algorithm = "Rebin"
    ...
    def xtest_that_compute_cut_returns_a_result_with_the_expected_size_for_normalized_psd_integration_data(
        self,
    ):
        normalized = True
    ...
    def test_that_compute_cut_returns_a_result_with_the_expected_size_for_normalized_non_psd_integration_data(
        self,
    ):
        normalized = True
        algorithm = "Integration"
    ...
    def test_that_compute_cut_returns_the_expected_size_for_non_psd_integration_data(
        self,
    ):
        normalized = False
        algorithm = "Integration"
    ...
    def test_that_compute_cut_returns_the_expected_size_for_a_rebin_cut_axis_in_units_of_2theta(
        self,
    ):
        non_psd_workspace = create_simulation_workspace(
            "Direct", "non_psd_ws", psd=False
        )

        cut = compute_cut(
            non_psd_workspace.raw_ws,
            self.theta_axis,
            self.e_axis,
            "Direct",
            False,
    ...
    def test_PyExec_will_execute_the_compute_cut_function(
        self, mock_compute_cut, mock_get_property, mock_set_property
    ):
        q_axis = {
            "units": MockProperty("|Q|"),
            "start": MockProperty(0.1),
            "end": MockProperty(3.1),
            "step": MockProperty(0.1),
            "e_unit": MockProperty("meV"),
        }
        e_axis = {
            "units": MockProperty("DeltaE"),
            "start": MockProperty(-10),
            "end": MockProperty(15),
            "step": MockProperty(1),
            "e_unit": MockProperty("DeltaE"),
        }
        ...
        mock_set_property.assert_called_with(
            "OutputWorkspace", mock_compute_cut.return_value
        )
