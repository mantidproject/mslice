...
class CLIHelperFunctionsTest(unittest.TestCase):
    ...
    @mock.patch(
        "mslice.plotting.globalfiguremanager.GlobalFigureManager.get_active_figure"
    )
    ...
    def test_that_update_overplot_checklist_works_as_expected_with_arb_nuclei(
        self, gaf
    ):
    ...
    def test_that_process_axis_logs_a_width_warning_if_its_smaller_than_the_x_data_width_in_the_workspace(
        self,
    ):
        ...
        with self._caplog.at_level(logging.WARNING):
            return_value = _process_axis(axis, fallback_index, workspace)
            self.assertEqual(
                return_value, Axis(units="DeltaE", start=-0.5, end=0.5, step=0.01)
            )
        self.assertEqual(
            "The DeltaE step provided (0.0100) is smaller than the data step in the workspace (1.0000). "
    ...
    def test_that_process_axis_does_not_log_a_warning_for_an_axis_index_of_one(self):
        ...
        with self._caplog.at_level(logging.WARNING):
            return_value = _process_axis(axis, fallback_index, workspace)
            self.assertEqual(
                return_value, Axis(units="|Q|", start=-0.5, end=0.5, step=0.01)
            )
