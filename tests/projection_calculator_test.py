...
class ProjectionCalculatorTest(TestCase):
    def setUp(self):
        self.non_psd_workspace = create_simulation_workspace(
        ...
        self.c_workspace = mantid_algorithms.CreateSampleWorkspace(
            NumBanks=1,
            BankPixelWidth=5,
            XMin=0.1,
            XMax=3.1,
    ...
    def test_available_axes(self):
        self.assertEqual(
            self.projection_calculator.available_axes(), ["|Q|", "2Theta", "DeltaE"]
        )

    def test_validate_workspace(self):
        self.assertEqual(
            self.projection_calculator.validate_workspace(self.psd_workspace), None
        )
        self.assertRaises(
            (AttributeError, TypeError),
            self.projection_calculator.validate_workspace,
            self.c_workspace,
        )
    ...
    def test_projection_calculation(self):
        self.assertRaises(
            RuntimeError,
            self.projection_calculator.calculate_projection,
            self.non_psd_workspace,
            "|Q|",
            "DeltaE",
        )
        self.assertRaises(
        ...
        result = self.projection_calculator.calculate_projection(
            self.psd_workspace, "|Q|", "DeltaE"
        )
        self.assertEqual(type(result), PixelWorkspace)
        result = self.projection_calculator.calculate_projection(
            self.psd_workspace, "DeltaE", "2Theta"
        )
        self.assertEqual(type(result), PixelWorkspace)
