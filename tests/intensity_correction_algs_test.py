...
class IntensityCorrectionAlgsTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.sim_scattering_data = (
            np.linspace(0.002, 1.5, 30 * 25).reshape(30, 25).transpose()
        )
        cls.e_axis = Axis(
            "DeltaE",
            cls.sim_scattering_data[0][0],
            cls.sim_scattering_data[24][29],
            0.002 * 25,
        )
        cls.q_axis = Axis(
            "|Q|", cls.sim_scattering_data[0][0], cls.sim_scattering_data[24][0], 0.002
        )
        ...
        cls.test_psd = CreateMDHistoWorkspace(
            OutputWorkspace="test_psd",
            Dimensionality=2,
            Extents=[0.1, 3.1, 1.0, 25.0],
            NumberOfBins=[25, 30],
            Names="Energy transfer,q",
            Units="meV,Angstrom^-1",
            SignalInput=cls.test_ws.get_signal(),
            ErrorInput=cls.test_ws.get_error(),
        )
    ...
    def test_compute_d2sigma(self, ws_handle_mock):
        ...
        d2sigma = compute_d2sigma(
            self.test_ws, self.e_axis, self.test_ws.e_fixed
        ).get_signal()
    ...
    def test_compute_symmetrised(self):
        symmetrised = compute_symmetrised(
            self.test_ws, 10, self.e_axis, False
        ).get_signal()
    ...
    def test_compute_symmetrised_rotated_psd(self):
        ...
        symmetrised = compute_symmetrised(
            self.test_ws, 10, self.e_axis, True
        ).get_signal()
        self.assertAlmostEqual(symmetrised[0][0], 0.002, 6)
        self.assertAlmostEqual(symmetrised[15][10], 0.532, 6)
        self.assertAlmostEqual(symmetrised[24][29], 1.5, 6)
    ...
    def test_compute_gdos_slice(self):
        gdos = slice_compute_gdos(
            self.test_ws, 10, self.q_axis, self.e_axis, self.rotated
        ).get_signal()
    ...
    def test_cut_compute_gdos_pixel(
        self, ws_handle_mock, cut_compute_gdos_pixel_mock, pixel_ws_init_mock
    ):
    ...
    def test_cut_compute_gdos_impl(self):
        self._internal_tst_cut_compute_gdos_impl(
            _cut_compute_gdos, [call(self.test_ws.parent)]
        )
    ...
    @patch("mslice.models.intensity_correction_algs.get_workspace_handle")
    def _internal_tst_cut_compute_gdos_impl(
        self,
        test_fn,
        ws_handle_calls,
        ws_handle_mock,
        ...
        parent_workspace.limits = {
            self.q_axis.units: [0.1, 3.1, 0.1],
            self.e_axis.units: [0.1, 6.2, 0.2],
        }
        ...
        slice_compute_gdos_mock.assert_called_once_with(
            parent_workspace, 10, self.q_axis, self.e_axis, self.rotated
        )
        ...
        reduce_bins_mock.assert_called_once_with(
            parent_workspace.__truediv__().__imul__().__imul__(),
            self.algorithm,
            self.q_axis,
            self.e_axis,
            1,
    ...
    def test_get_slice_axis_icut_alligns_bins(self):
        ...
        self.assertEqual(
            Axis(
                self.q_axis.units, 0.0005, 0.0505, self.q_axis.step, self.q_axis.e_unit
            ),
            ret_axis,
        )
    ...
    def _internal_tst_reduce_bins(
        self, algorithm, rotated, use_psd, createMDHistoWorkspace_mock, cutalgo_mock
    ):
        if rotated:
            units = "meV,Angstrom^-1"
            names = "Energy transfer,q"
        ...
        self.assertEqual(
            f"{cut_x_dim.getMinimum()},{cut_x_dim.getMaximum()},{cut_y_dim.getMinimum()},{cut_y_dim.getMaximum()}",
        ...
        if use_psd:
            ...
            signal_result = (
                np.nansum(selected_test_ws.get_signal(), int(rotated), keepdims=True)
                * integration_factor
            )
            error_result = (
                np.sqrt(
                    np.nansum(
                        selected_test_ws.get_variance(), int(rotated), keepdims=True
                    )
                )
                * integration_factor
            )
            shape_string = (
                f"{signal_result.shape[1]},{signal_result.shape[0]}"
                if rotated
                else f"{signal_result.shape[0]},{signal_result.shape[1]}"
            )
            self.assertEqual(
                shape_string, createMDHistoWorkspace_mock.call_args[1]["NumberOfBins"]
            )
            self.assertTrue(
                np.allclose(
                    signal_result,
                    createMDHistoWorkspace_mock.call_args[1]["SignalInput"],
                    atol=1e-08,
                )
            )
            self.assertTrue(
                np.allclose(
                    error_result,
                    createMDHistoWorkspace_mock.call_args[1]["ErrorInput"],
                    atol=1e-08,
                )
            )
        else:
