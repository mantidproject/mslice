...
class AxisTest(unittest.TestCase):
    ...
    def test_validate_step_against_workspace_returns_none_if_the_cut_width_is_larger_than_the_data_x_width(
        self, mock_get_axis_step
    ):
        mock_ws = MagicMock()
    ...
    @patch("mslice.models.axis.get_axis_step")
    def test_validate_step_against_workspace_returns_error_if_the_cut_width_is_smaller_than_the_data_x_width(
        self, mock_get_axis_step
    ):
        mock_ws = MagicMock()
        ...
        axis = Axis("DeltaE", "0", "5", "0.1", "cm-1")
        self.assertEqual(
            "The DeltaE step provided (0.1000) is smaller than the data step in the "
            "workspace (0.2000). Please provide a larger DeltaE step.",
            axis.validate_step_against_workspace(mock_ws),
        )

    @patch("mslice.models.axis.get_axis_step")
    def test_validate_step_against_workspace_returns_none_if_the_cut_width_is_smaller_than_the_data_x_width_but_in_tol(
        self, mock_get_axis_step
    ):
        mock_ws = MagicMock()
