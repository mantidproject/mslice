...
def _string_to_axis(string):
    ...
    if len(axis) != 4 and len(axis) != 5:
        raise ValueError(
            "axis should be specified in format <name>,<start>,<end>,<step_size>(,<e_unit>)"
        )
...
def _string_to_integration_axis(string):
    ...
    if len(axis_str) < 3:
        raise ValueError(
            "axis should be specified in format <name>,<start>,<end>(,<step>,<e_unit>)"
        )
    elif len(axis_str) == 3:
        valid_axis = Axis(
            axis_str[0],
            axis_str[1],
            axis_str[2],
            str(float(axis_str[2]) - float(axis_str[1])),
        )
    elif len(axis_str) == 4:
        valid_axis = Axis(axis_str[0], axis_str[1], axis_str[2], axis_str[3])
    else:
        valid_axis = Axis(
            axis_str[0], axis_str[1], axis_str[2], axis_str[3], axis_str[4]
        )
    return valid_axis


def _process_axis(
    axis, fallback_index, input_workspace, string_function=_string_to_axis
):
...
def _rescale_energy_cut_plot(presenter, cuts, new_e_unit):
    ...
    cuts_copy = copy.deepcopy(
        cuts
    )  # Because run_cut will overwrite the cuts cache for plot_over=True
...
def is_cut(*args):
    ...
    if (
        ...
        and sum(
            [
                args[0].raw_ws.getDimension(i).getNBins() != 1
                for i in range(args[0]._raw_ws.getNumDims())
            ]
        )
        == 1
    ):
...
def append_visible_handle_and_label(
    visible_handles, handles, visible_labels, labels, idx: int
):
...
def _correct_intensity(
    ...
    try:
        intensity_correction = IntensityCache.get_intensity_type_from_desc(
            intensity_correction
        )
    except ValueError:
...
def _check_sample_temperature(sample_temperature, workspace_name):
    ...
    if throw_error:
        raise SampleTempValueError(
            "No, or invalid, sample temperature provided", workspace_name
        )
