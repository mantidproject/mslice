...
class InteractiveCut(object):
    ...
    def plot_cut(self, x1, x2, y1, y2, store=False):
        if x2 > x1 and y2 > y1:
            ...
            units = (
                ...
                if self.horizontal
                else self._canvas.figure.gca().get_xaxis().units
            )
            integration_axis = Axis(
                units, integration_start, integration_end, 0, self._en_unit
            )
            ...
            cut = Cut(
                ...
                None,
                None,
                sample_temp=self.slice_plot.temp,
                e_fixed=workspace.e_fixed,
            )
            cut.parent_ws_name = self._ws_title
            cut.icut = self
            self._cut_plotter_presenter.plot_interactive_cut(
                workspace, cut, store, self.slice_plot.intensity_type
            )
    ...
    def get_cut_parameters(self, pos1, pos2):
        ...
        units = (
            ...
            if self.horizontal
            else self._canvas.figure.gca().get_yaxis().units
        )
        step = (
            get_limits(get_workspace_handle(self._ws_title), units)[2]
            * self._en_from_meV
        )
    ...
    def save_cut(self):
        ...
        return output_workspace_name(
            str(self._ws_title), integration_start, integration_end
        )
