...
def compute_slice(selected_workspace, x_axis, y_axis, norm_to_one, store_in_ADS=True):
    ...
    slice = mantid_algorithms.Slice(
        OutputWorkspace="__" + workspace.name,
        InputWorkspace=workspace,
        XAxis=x_axis.to_dict(),
        YAxis=y_axis.to_dict(),
        PSD=workspace.is_PSD,
        EMode=workspace.e_mode,
        NormToOne=norm_to_one,
...
def compute_recoil_line(ws_name, axis, relative_mass=None):
    ...
    if is_momentum(axis.units):
        ...
        line = (
            np.square(momentum_transfer * 1.0e10 * constants.hbar)
            / (2 * relative_mass * constants.neutron_mass)
            / (constants.elementary_charge / 1000)
        )
    elif is_twotheta(axis.units):
        tth = x_axis * np.pi / 180.0
