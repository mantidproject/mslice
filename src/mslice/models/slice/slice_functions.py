...
def compute_slice(selected_workspace, x_axis, y_axis, norm_to_one, store_in_ADS=True):
    ...
    output_name = WorkspaceNameHandler(workspace.name).get_name(hide_from_ADS=True)
    slice = mantid_algorithms.Slice(OutputWorkspace=output_name, InputWorkspace=workspace,
