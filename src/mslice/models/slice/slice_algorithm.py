...
class Slice(PythonAlgorithm):
    def PyInit(self):
        ...
        self.declareProperty(
            PropertyManagerProperty("XAxis", {}, direction=Direction.Input),
            doc="MSlice Axis object as a dictionary",
        )
        self.declareProperty(
            PropertyManagerProperty("YAxis", {}, direction=Direction.Input),
            doc="MSlice Axis object as a dictionary",
        )
    ...
    def PyExec(self):
        ...
        e_axis = (
            0 if "DeltaE" in x_axis.units else (1 if "DeltaE" in y_axis.units else None)
        )
        if e_axis is not None:
            e_scale = EnergyUnits(
                x_axis.e_unit if e_axis == 0 else y_axis.e_unit
            ).factor_from_meV()
        ...
        if self.getProperty("PSD").value:
            ...
            if e_axis is not None and e_scale != 1.0:
                scale = [1.0, e_scale] if e_axis == 1 else [e_scale, 1.0]
                slice = TransformMD(InputWorkspace=slice, Scaling=scale)
        else:
            e_mode = self.getProperty("EMode").value
            slice = self._compute_slice_nonPSD(
                workspace, x_axis, y_axis, e_mode, norm_to_one
            )
        ...
        if axes[q_axis].units == "|Q|":
            if "Indirect" in e_mode and workspace.run().hasProperty("Efix"):
                thisslice = SofQW3(
                    ...
                    EMode=e_mode,
                    StoreInADS=False,
                    EFixed=workspace.run().getProperty("Efix").value,
                )
            else:
                thisslice = SofQW3(
                    InputWorkspace=workspace,
            ...
            thisslice = ConvertSpectrumAxis(
                InputWorkspace=workspace, Target="Theta", StoreInADS=False
            )
            thisslice = Rebin2D(
                InputWorkspace=thisslice,
                Axis1Binning=ebin,
                Axis2Binning=qbin,
                StoreInADS=False,
            )
