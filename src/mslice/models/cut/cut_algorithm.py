...
class Cut(PythonAlgorithm):
    def PyInit(self):
        ...
        self.declareProperty(
            PropertyManagerProperty("CutAxis", {}, direction=Direction.Input),
            doc="MSlice Axis object as a dictionary",
        )
        self.declareProperty(
            PropertyManagerProperty("IntegrationAxis", {}, direction=Direction.Input),
            doc="MSlice Axis object as a dictionary",
        )
        ...
        self.declareProperty(
            "Algorithm", "Rebin", StringListValidator(["Rebin", "Integration"])
        )
        self.declareProperty(
    ...
    def PyExec(self):
        ...
        cut_axis = Axis(
            cut_dict["units"].value,
            cut_dict["start"].value,
            cut_dict["end"].value,
            cut_dict["step"].value,
        ...
        int_axis = Axis(
            int_dict["units"].value,
            int_dict["start"].value,
            int_dict["end"].value,
            int_dict["step"].value,
        ...
        if "DeltaE" in cut_axis.units and cut_axis.scale != 1.0:
            cut = TransformMD(
                InputWorkspace=cut,
                Scaling=[EnergyUnits(cut_axis.e_unit).factor_from_meV()],
            )
        attribute_to_log(
            {
                "axes": [cut_axis, int_axis],
...
def compute_cut(
    selected_workspace, cut_axis, integration_axis, e_mode, PSD, is_norm, algo
):
    if PSD:
        cut = _compute_cut_PSD(selected_workspace, cut_axis, integration_axis, algo)
    else:
        cut = _compute_cut_nonPSD(
            selected_workspace, cut_axis, integration_axis, e_mode, algo
        )
    if is_norm:
...
def _compute_cut_PSD(selected_workspace, cut_axis, integration_axis, algo):
    ...
    cut_binning = " ,".join(
        map(str, (cut_axis.units, cut_axis.start_meV, cut_axis.end_meV, n_steps))
    )
    integration_binning = (
        integration_axis.units
        + ","
        + str(integration_axis.start_meV)
        + ","
        + str(integration_axis.end_meV)
        + (",1" if "Rebin" in algo else ",100")
    )

    ws = BinMD(
        InputWorkspace=selected_workspace,
        AxisAligned="1",
        AlignedDim0=integration_binning,
        AlignedDim1=cut_binning,
        StoreInADS=False,
    )
    if "Integration" in algo:
        ...
        norm_fac = (np.sum(ws.getNumEventsArray() != 0.0, axis=0) / 100) * (x1 - x0)
...
def _compute_cut_nonPSD(selected_workspace, cut_axis, integration_axis, emode, algo):
    cut_binning = " ,".join(
        map(str, (cut_axis.start_meV, cut_axis.step_meV, cut_axis.end_meV))
    )
    int_binning = " ,".join(
        map(
            str,
            (
                integration_axis.start_meV,
                integration_axis.end_meV - integration_axis.start_meV,
    ...
    if is_momentum(cut_axis.units):
        ws_out = _cut_nonPSD_momentum(
            cut_binning, int_binning, emode, selected_workspace, algo
        )
        idx = 1
        ...
        ws_out = _cut_nonPSD_momentum(
            int_binning, cut_binning, emode, selected_workspace, algo
        )
    else:
    ...
    _tmpws = CreateSimulationWorkspace(
        Instrument="MAR",
        BinParams=[-1, 1, 1],
        UnitX="DeltaE",
    ...
    AddSampleLog(
        _tmpws, LogName="Ei", LogText="3.", LogType="Number", EnableLogging=False
    )
    _tmpws = ConvertToMD(
        _tmpws,
        EnableLogging=False,
    ...
    ws_out = CreateMDHistoWorkspace(
        SignalInput=ws_out.extractY(),
...
def _cut_nonPSD_general(ax1_binning, ax2_binning, selected_workspace, algo):
    if "Integration" in algo:
        ...
        if np.abs((ax1[2] - ax1[0]) - ax1[1]) < 0.0001:
            ...
            ws_out = Rebin(
                InputWorkspace=ws_out, Params=ax2_binning, EnableLogging=False
            )
        else:
            ws_out = Rebin(
                InputWorkspace=selected_workspace,
        ...
        ws_out = Rebin2D(
            InputWorkspace=selected_workspace,
            Axis1Binning=ax1_binning,
            Axis2Binning=ax2_binning,
            StoreInADS=False,
...
def _cut_nonPSD_theta(ax1_binning, ax2_binning, selected_workspace, algo):
    converted_nonpsd = ConvertSpectrumAxis(
        InputWorkspace=selected_workspace, Target="theta", StoreInADS=False
    )
...
def _cut_indirect_or_direct(q_binning, e_binning, emode, selected_workspace):
    if "Indirect" in emode and selected_workspace.run().hasProperty("Efix"):
        ws_out = SofQW3(
            ...
            EMode=emode,
            StoreInADS=False,
            EFixed=selected_workspace.run().getProperty("Efix").value,
            EnableLogging=False,
        )
    else:
        ws_out = SofQW3(
...
def _cut_nonPSD_momentum(q_binning, e_binning, emode, selected_workspace, algo):
    if "Integration" in algo:
        ...
        if np.abs((qbins[2] - qbins[0]) - qbins[1]) < 0.0001:
            qbinstr = ",".join(
                map(str, [qbins[0], (qbins[2] - qbins[0]) / 100.0, qbins[2]])
            )
            ws_out = _cut_indirect_or_direct(
                qbinstr, e_binning, emode, selected_workspace
            )
            ws_out = Transpose(InputWorkspace=ws_out, EnableLogging=False)
            ws_out = Integration(
                InputWorkspace=ws_out,
            ...
            ebinstr = ",".join(
                map(str, [ebins[0], (ebins[2] - ebins[0]) / 100.0, ebins[2]])
            )
            ws_out = _cut_indirect_or_direct(
                q_binning, ebinstr, emode, selected_workspace
            )
            ws_out = Integration(
                InputWorkspace=ws_out,
                RangeLower=ebins[0],
        ...
        ws_out = _cut_indirect_or_direct(
            q_binning, e_binning, emode, selected_workspace
        )
    return ws_out
