...
class MantidProjectionCalculator(ProjectionCalculator):
    ...
    def validate_workspace(self, ws):
        ...
        try:
            ...
            raise TypeError(
                "Input workspace for projection calculation must be a reduced "
    ...
    def calculate_projection(self, input_workspace, axis1, axis2):
        ...
        if axis1 == MOD_Q_LABEL or axis2 == MOD_Q_LABEL:
            ...
            output_workspace_name = workspace.name + (
                "_QE" if axis1 == MOD_Q_LABEL else "_EQ"
            )
        elif axis1 == THETA_LABEL or axis2 == THETA_LABEL:
            projection_type = "Theta"
            output_workspace_name = workspace.name + (
                "_ThE" if axis1 == THETA_LABEL else "_ETh"
            )
        else:
            raise NotImplementedError(
                " Axis '%s' not recognised. Must be '|Q|' or '2Theta'."
                % (axis1 if axis1 != DELTA_E_LABEL else axis2)
            )

        new_ws = mantid_algorithms.MakeProjection(
