...
class MakeProjection(PythonAlgorithm):
    def PyInit(self):
        ...
        self.declareProperty(
            ...
            doc="Q or Theta projection",
        )
        self.declareProperty(
            IMDEventWorkspaceProperty("OutputWorkspace", "", direction=Direction.Output)
        )
    ...
    def _getDetWS(self, input_workspace):
        ...
        wsdet = PreprocessDetectorsToMD(
            OutputWorkspace=wsdet, StoreInADS=False, InputWorkspace=input_workspace
        )
        return wsdet
    ...
    def _calcQEproj(self, input_workspace, emode, axis1, axis2):
        ...
        # Otherwise first run SofQW3 to rebin it in |Q| properly before calling ConvertToMD with CopyToMD
            ...
            retval = ConvertToMD(
                InputWorkspace=retval,
                QDimensions="CopyToMD",
                PreprocDetectorsWS="-",
                dEAnalysisMode=emode,
                StoreInADS=False,
            )
