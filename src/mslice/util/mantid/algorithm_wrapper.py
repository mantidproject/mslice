...
def wrap_algorithm(algorithm):
    def alg_wrapper(*args, **kwargs):
        ...
        for ky in [k for k in kwargs.keys() if 'Workspace' in k]:
            is_hidden_msl_ws = WorkspaceNameHandler(kwargs[ky]).assert_name(is_hidden_from_ADS=True, has_mslice_signature=True)
            if isinstance(kwargs[ky], string_types) and not is_hidden_msl_ws:
