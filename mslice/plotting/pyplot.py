...
from matplotlib.cbook import is_string_like, is_numlike, silent_list
...
if float('.'.join(matplotlib.__version__.split('.')[:2])) >= 1.5:
...
def imshow(X, cmap=None, norm=None, aspect=None, interpolation=None, alpha=None,
    ...
    try:
        ...
        ret = ax.imshow(X, cmap=cmap, norm=norm, aspect=aspect,
                        ...
                        imlim=imlim, resample=resample, url=url,
                        **kwargs)
    finally:
        ax.hold(washold)
    sci(ret)
