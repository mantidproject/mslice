...
class PixelWorkspace(Workspace):
    ...
    def __init__(self, pixel_workspace):
        self.pixel_workspace = pixel_workspace
    ...
    def get_histogram_workspace(self):
        if self.histogram_workspace is None:
            ...
            for x in range(6):
                try:
                    dim = self.pixel_workspace.getDimension(x)
                    dim_info = dim.getName() + ',' + str(dim.getMinimum()) + ',' + str(dim.getMaximum()) + ',' + str(10)
                except RuntimeError:
                    dim_info = None
                dim_values.append(dim_info)
            histo_workspace = BinMD(InputWorkspace=self.pixel_workspace, OutputWorkspace="output",
